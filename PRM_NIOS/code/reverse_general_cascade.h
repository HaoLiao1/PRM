...
/// RR set with distance  lable
typedef std::vector< std::pair<int, int> > RRDVec;
/// RR set with time lable
typedef std::vector< std::pair<int, int> > RRTVec;
...
public:
	...
	double ReversePropagate(int num_iter, int target,  //added
		std::vector< std::pair< RRVec, int > >& outRRSets,
		int& outEdgeVisited,
		int NodeNumber)
	{
		// MI_STATIC_ASSERT(has_mem_GetNeighborCount<graph_type>::value, "graph_type should has member GetNeighbor");
		...
		for (int it = 0; it < num_iter; it++)
		{
			...
			std::pair< RRVec, int > RRWithTime;
			RRWithTime.first = RR;
			RRWithTime.second = random.RandInt(0, NodeNumber); 
			
			outRRSets.push_back(RRWithTime);
		}
		return (double)resultSize / (double)num_iter;
	}
