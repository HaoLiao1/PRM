...
void CIMM::Build(graph_type& gf, int k, cascade_type& cascade, double eps /* = 0.1 */, double ell /* = 1.0 */, double delta /* = 1.0*/)
{
	...
	for (int r = 1; r < maxRounds; r++) {
		...
		if (table.size() < theta) {
			...
			std::cout << _EstimateInfl(budget) << endl;
			for (int i = 0; i < n; i++)
			{
...
void ShapleyInfl::Shapley_Build(graph_type& gf, cascade_type& cascade, GeneralCascade& gc, double eps/*=0.1*/, double ell/*=1.0*/, int topk /*=50*/, bool isSingleInf /* = false */)
{
	...
	std::cout << "Phase 1: Estimate the number of RR sets needed" << endl;
	pctimer2.SetTimeEvent("phase1_start");
	double LB = 1.0;   // lower bound

	// use the IMM sampling method to estimate lower bound LB
	std::cout << "lower bound LB estimated using the sampling method similar to IMM" << endl;
	double epsprime = eps * sqrt(2.0); // eps'
	...
	for (int r = 1; r < maxRounds; r++) {
		...
		if (!isSingleInf) {
			std::cout << "No. " << topk << " largest Shapley value = " << shpv << "\t current round = " << ((nNewSamples > 0) ? nNewSamples : 0) << "\t total round = " << totalRRSetSoFar << endl;
		}
		else {
			std::cout << "No. " << topk << " largest SNI value = " << shpv << "\t current round = " << ((nNewSamples > 0) ? nNewSamples : 0) << "\t total round = " << totalRRSetSoFar << endl;
		}
		stept.SetTimeEvent("step_end");
		steptimers.push_back(stept);
	...
	if (!isSingleInf) {
		std::cout << "Phase 1 ends: Estimated lower bound of the k-th largest Shapley value: " << LB << endl;
	}
	else {
		std::cout << "Phase 1 ends: Estimated lower bound of the k-th largest SNI value: " << LB << endl;
	}
	std::cout << "              Number of RR sets needed for Phase 2: " << stheta << endl;
	...
	if (!isSingleInf) {
		std::cout << "Phase 2: generate RR sets and estimate Shapley values of all nodes, theta = " << stheta << endl;
	}
	else {
		std::cout << "Phase 2: generate RR sets and estimate SNI values of all nodes, theta = " << stheta << endl;
	}
	...
	if (!isSingleInf) {
		std::cout << "k-th largest Shapley value = " << shapleyVId[topk - 1].value << "; EPT = " << EPT << endl;
	}
	else {
		std::cout << "k-th largest SNI value = " << shapleyVId[topk - 1].value << "; EPT = " << EPT << endl;
	}
